...
def extract_kw(gA):
    kwL = gA.xpath("summary[@xml:lang='ja']/keywordList/keyword")
    if kwL == []:
        result = "NotFound"
    else:
        text = ''
...
def read_xml(xml, year):
    ...
    for gA in tree.xpath('/grantAwards/grantAward'):
        ...
        kwlist = extract_kw(gA)

        row = [awardid, title, purpose, abstract, initial, performance, achieve, kwlist]
...
def ap():
    parser = argparse.ArgumentParser(description='KAKENから課題番号とテキスト（課題名や研究開始時の研究の概要，そしてキーワードなど）を抽出する。開始年度のみ指定可能。')
